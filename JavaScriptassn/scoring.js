...
$(document).ready(function () {
  ...
  function get_achievement() {
    ...
    sum = sum + subject_points[3];
    sum = sum + subject_points[4];
    // let averageIndicate=sum_indicate/sum_indicate.length();
  console.log("totalsum is " + sum);
  ...
  function get_pass_or_failure() {
    ...
    for (var i = 0; i < subject_points.length; i++) {
      ...
      if (subject_points[i]<60){
        judge="Fail";
        //break;to break out so as not to loop through everything
      }
    }
  ...
  //  let result=( "Your grade is "+ achievement + " You " + judgement);
  //  console.log(result);
  //  $("#btn-declaration").text(result);
  });
});
